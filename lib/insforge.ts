...
export async function listRoomsWithStats(): Promise<RoomWithStats[]> {
  ...
  // Fetch succeeded jobs once and bucket latest finished_at per page
  // in memory. The room list can include hundreds of active pages,
  // so this must stay O(1) in PostgREST round-trips rather than
  // issuing one snapshot_jobs query per tracked page.
  ...
  if (activePages.length > 0) {
    ...
    for (const job of jobs) {
      if (activePageIds.has(job.tracked_page_id) && jobsMap[job.tracked_page_id] === undefined) {
