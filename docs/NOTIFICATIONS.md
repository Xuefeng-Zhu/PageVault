...
- The single-statement CLI limitation means the migration is applied as 4 separate `npx @insforge/cli db query` calls: one per `create or replace function`, one per `revoke all`, one per `grant execute`. See `db/migrations/2026-06-02-notification-advisory-lock.sql` for the full SQL.
