...
**Inputs:** raw research notes in `/tmp/pv-research/` (collected by prior worker, t_df622d29, who ran out of iteration budget); `docs/mvp-cut.md`; `docs/stories.md` (PageVault source-of-truth for product claims)
...
- **Tech differentiator vs. PageVault:** ChangeTower's value is "HTML snapshots + condition-based alerts + multi-user alerting + full page archiving." PageVault's claim to differentiate is the *interpreted* change brief (LLM-generated `summary`, `severity`, `recommended_actions[]` per stories.md US-004), not the raw snapshot. ChangeTower does have AI-powered monitoring in its tier matrix, but it is a relevance flag, not a structured explanation payload.
...
**Claim in PRD (`docs/stories.md` + `docs/mvp-cut.md`):** Maya is a single senior product manager, runs competitor monitoring on 8–12 URLs, wants a Slack ping with an evidence link, and her MTTD target is <6 hours for medium/high-severity changes (stories.md G1, US-001 to US-004; mvp-cut.md §2).
...
Anchor: Visualping's published pricing (G2 profile, scraped 2025-02-07). Where the G2 page and the TechRadar review disagree on the Personal tier ($10 vs. $50), the G2 page is used because it is the more recent of the two scrapes and the prices align across the G2 card and the G2 product page (both scraped in the same Wayback capture).
...
|---|---|---|---|---|
| **Evidence durability (raw, hashed, append-only chain)** | **WIN** — InsForge Storage, SHA-256 on every snapshot, previous + new stored together, BoxSystemError fails loud (no mock fallback). stories.md US-006. | **PARTIAL** — visual diff + screenshot attached to email alert, but no published "audit-trail export" feature. | **PARTIAL** — "Full page archiving" on Business tier, 6mo retention. Better than Visualping on this row, worse than PageVault. | **LOSE** — varies. If a team built a proper S3 + hash pipeline, this is fine. If they didn't (most), it isn't. |
| **LLM interpretation (structured summary + severity + recommended_actions)** | **WIN** — `ai_explanations` row with `{summary, severity, categories[], recommended_actions[]}` from an OpenAI-compat model. stories.md US-004. | **PARTIAL** — Visualping AI does summaries and a "mind reader" relevance filter (G2 profile) but the deliverable is still a screenshot + email, not a structured payload. TechRadar review (2025) calls it out as a differentiator. | **LOSE** — keyword alerts and "AI-powered monitoring" listed as a feature in the tier matrix, but no structured brief documented. | **LOSE** — manual. |
| **Self-serve (no sales call to start)** | **WIN** — Demo Mode + "Load Demo" button (PRD §4.7, stories.md US-015). Single-user MVP-1 ships with no signup. | **WIN** — Free tier, no card required. | **WIN** — Free tier (3 pages, daily). | **LOSE** — engineer-time is the only cost. |
...
| **6h MTTD out-of-box (detection <6h on a competitor's pricing page change)** | **PARTIAL / LOSE** — MVP-1 is manual trigger only. MVP-2 adds hourly cron, which clears the 6h bar. mvp-cut.md §3. | **WIN** — every-5-min checks on paid plans; Visualping's default is 60-min on Personal. | **WIN** — every-20-min on Business tier. | **PARTIAL** — depends on the cron schedule the in-house team set. |
...
| **Multi-source (URL + change feed + API + manual paste)** | **PARTIAL** — URL monitoring in MVP-1; RSS / API ingest in v2. mvp-cut.md §1 row 24. | **PARTIAL** — URL monitoring + Chrome extension + Button (for site visitors); no RSS ingest documented. | **PARTIAL** — URL monitoring + Domain scanning on Enterprise. | **PARTIAL** — depends on the in-house build. |
...
- **Detection latency (6h MTTD) is a MVP-2 promise, not a MVP-1 promise.** Visualping and ChangeTower ship hourly or better on their entry paid tiers. PageVault's MVP-1 is manual-trigger only, and the cron that closes the 6h gap is in MVP-2 (mvp-cut.md §3, "NOT in MVP-1: Scheduled scans (cron)"). For the 6h-MTTD buyer, Visualping's $10/mo Personal tier is the right answer today; PageVault is the right answer in 6+ weeks. **This is the single most important honest LOSE in the table.**
...
- PageVault PRD claims cited: `docs/PRD.md` (K-01, t_761e9d61) — referenced in `docs/mvp-cut.md` and `docs/stories.md` (PRD itself is not in the repo; this is documented in the mvp-cut.md header caveat). The mvp-cut.md and stories.md files are the operative product sources.
- PageVault features cited: `docs/stories.md` US-001, US-002, US-004, US-006, US-007, US-009, US-015 · `docs/mvp-cut.md` §1, §3
- Competitor claims cited: same Section 1 sources.

### Open questions / follow-up cards

- **Re-fire persona validation with browser-stealth** (assigned profile: research, with `t_…` id to be created; parent: t_af403bcf). Acceptance: ≥3 first-person PM artifacts. Without this, the MVP-1 cut's "Maya-pain-anchored" value scoring (mvp-cut.md §1 value column) is single-sourced.
...
- **Visualping G2 vs. TechRadar price disagreement** (Personal tier: $10 vs. $50). G2 is the more recent of the two scrapes and is used as the primary anchor; if the price has moved since, the §3 anchor needs re-validation. The visualping.io live page is JS-rendered and the Wayback snapshots do not include the price block.
- **The PRD itself (`docs/PRD.md`) is not in the repo.** The mvp-cut.md and stories.md files reference it (K-01, t_761e9d61) but the file itself was not found in the working tree. The product claims cited in §4 are grounded in stories.md and mvp-cut.md, both of which are in the working tree.
